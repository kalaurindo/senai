...
def criar_interface():
    ...
    saldo_label = tk.Label(root, text="Saldo: R$ 0.00", width=30)
    ...
    btn_depositar = tk.Button(root, text="Depositar", command=lambda: depositar(saldo_label, valor_entry), width=15)
    btn_depositar.grid(row=2, column=0)

    btn_sacar = tk.Button(root, text="Sacar", command=lambda: sacar(saldo_label, valor_entry),width=15)
